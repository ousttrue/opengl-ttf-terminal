...
static int pty_setup_child(int slave, unsigned short term_width,
...
pid_t shl_pty_open(shl_pty **out, shl_pty_input_cb cb, void *data,
...
bool shl_pty_is_open(shl_pty *pty) { return pty->fd >= 0; }

int shl_pty_get_fd(shl_pty *pty) { return pty->fd; }

pid_t shl_pty_get_child(shl_pty *pty) { return pty->child; }
...
int shl_pty_resize(shl_pty *pty, unsigned short term_width,
                   unsigned short term_height) {
  struct winsize ws;
  int r;
...
void shl_pty_bridge_free(int bridge) { close(bridge); }
